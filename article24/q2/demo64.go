...
func main() {
	...
	box6.Store(v6)
	v6[1] = 4 // 注意，此处的操作不是并发安全的！，在Store()之后去操作存储的切片并不受value保护，相当于绕过了value
	...
	storee := func(v []interface{}) {
		replica := make([]interface{},len(v))
		copy(replica,v)
		box6.Store(replica)
	}
	_ = storee
	...
	fmt.Printf("Store %v to box6.\n", v6)
	store(v6) //store()将V6的副本存进了value里面，那么改变原V6并不会影响value里面的V6
	v6[2] = 5 // 此处的操作是安全的。
	fmt.Printf("The value load from box6 is %v.\n", box6.Load())
}
